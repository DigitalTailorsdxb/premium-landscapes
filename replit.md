...
- **Instant Quote Page (`quote.html`):** Revolutionary 5-step conversational quote system with progressive disclosure:
  ...
  - **Step 2 (Dual Mode):**
    ...
    - **Full Redesign Mode:** Simplified single textarea for customers to describe their dream garden. Includes helpful prompts for what to mention (materials, features, style, practical needs). No material selection UI - workflow creates custom design proposal based on requirements and budget. `project.notes`, `project.designPreferences`, and `design.designNotes` all contain the customer's vision text.
